...
import { getList, getItem } from "../api/cities";
import { PopupErrmsg } from "../components/PopupErrmsg";
import "../styles/listForm.css";
import { updateItem, addItem, deleteItem } from "../api/cities";
import { CityFormContent } from '../components/CityFormContent'
...
export function Cities() {
  ...
  if (!loaded && !loading) {
    async function load() {
      ...
      const emptyItem = await getItem(0); // To get select lists
      emptyItem.selectListCountries.unshift({
        text: "0",
  ...
  const popupTitle = item && item.id ? "Edit City" : "Add City";
  return (
    <>
...
function validateItem (item) {
  ...
  return item
    ? {
        errName: isNullOrEmpty(item.name) ? "City name can not be empty" : "",
        errCountry: item.countryId ? "" : "Country needs have a value",
      }
    : {};
}